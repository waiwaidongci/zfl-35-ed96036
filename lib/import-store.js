...
import { mutate, OPERATION, clone, getDefaultSiteId, listSites } from "./data-store.js";
...
function validateRow(row, index, existingIds, idsInImport, validSiteIds, defaultSiteId, defaultSiteName) {
  const errors = [];
  const warnings = [];
  ...
  if (row.siteId !== undefined && row.siteId !== null && row.siteId !== "") {
    ...
    } else if (!validSiteIds.has(row.siteId)) {
...
export async function previewImport(rows) {
  ...
  const validationResults = rows.map((row, i) => validateRow(row, i, existingIds, idsInImport, validSiteIds, defaultSiteId, defaultSiteName));
  ...
  for (const result of validationResults) {
    ...
    if (result.errors.some(e => e.code === "site_not_found")) {
      invalidSites.push({
        index: result.index,
        id: result.row ? result.row.id : null,
        siteId: result.row ? result.row.siteId : null,
        issues: result.errors.filter(e => e.code === "site_not_found")
      });
    }
  }
...
export async function confirmImport(token, options, ctx = {}) {
  ...
  const preCheckDb = await loadDb();
  const currentFingerprint = generateFingerprint(preCheckDb);
  ...
  const existingIdsPre = new Set(preCheckDb.batches.map(b => b.id));
  ...
  return mutate({
    ...
    mutator: (db) => {
      ...
      const defaultSite = getDefaultSiteId(db);
