...
export function calculateRetestPlan(batch, options = {}) {
  ...
  const baseDate = latestDate || now;
  suggestedRetestDate = new Date(baseDate.getTime() + retestIntervalDays * 24 * 60 * 60 * 1000);

  if (retestPriority === "none" && latestDate) {
    const standardDate = new Date(latestDate.getTime() + opts.standardRetestIntervalDays * 24 * 60 * 60 * 1000);
    suggestedRetestDate = standardDate;
  }
...
export async function generateRetestPlanReport(options = {}, siteIdParam = null) {
  ...
  const formatRetestItem = ({ batch, retestPlan }) => ({
    batchId: batch.id,
    siteId: batch.siteId || defaultSiteId,
    species: batch.species,
    collectionPlace: batch.collectionPlace,
    retestPriority: retestPlan.retestPriority,
    priorityScore: retestPlan.priorityScore,
    retestReasons: retestPlan.retestReasons,
    ruleTriggers: retestPlan.ruleTriggers,
    suggestedRetestDate: retestPlan.suggestedRetestDate,
    retestIntervalDays: retestPlan.retestIntervalDays,
    latestRate: retestPlan.latestRate,
    latestRateFormatted: retestPlan.latestRateFormatted,
    daysSinceLastTest: retestPlan.daysSinceLastTest,
    lastTestDate: retestPlan.lastTestDate,
    trendDirection: retestPlan.trendDirection,
    pendingReviewCount: retestPlan.pendingReviewCount,
    germinationCount: retestPlan.germinationCount
  });
  ...
  return {
    ...
    allAnalyses: analyses.map(({ batch, viabilityAnalysis, retestPlan }) => ({
      batchId: batch.id,
      siteId: batch.siteId || defaultSiteId,
      species: batch.species,
      latestRate: viabilityAnalysis.latestRate,
      latestRateFormatted: viabilityAnalysis.latestRate !== null ? `${(viabilityAnalysis.latestRate * 100).toFixed(1)}%` : null,
      trendDirection: viabilityAnalysis.trendDirection,
