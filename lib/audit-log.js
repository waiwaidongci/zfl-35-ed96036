import { loadAudit, loadAuditWithVersion, loadDb, clone, seed as dbSeed, getDefaultSiteId, filterBatchesBySite } from "./data-store.js";
...
export async function queryAuditLogs(filters = {}, siteIdParam = null) {
  ...
  if (!isGlobal) {
    const siteBatchIds = new Set(
      filterBatchesBySite(db.batches, effectiveSiteId, defaultSiteId).map(b => b.id)
    );
    if (siteBatchIds.size > 0) {
      logs = logs.filter(l => {
        const relatedBatchIds = new Set([
          ...(l.affectedBatches || []),
          l.entityType === "batch" ? l.entityId : null
        ].filter(Boolean));
        if (relatedBatchIds.size === 0) return false;
        for (const id of relatedBatchIds) {
          if (siteBatchIds.has(id)) return true;
        }
        return false;
      });
    } else {
  ...
  return {
    ...
    versions: {
      ...
      auditUpdatedAt: auditResult.updatedAt
    },
    logs,
    siteFilter: {
      ...
      note: isGlobal
        ? "全局审计日志（所有站点）"
        : siteIdParam
          ? `指定站点 ${effectiveSiteId} 的审计日志`
          : `未传 siteId，使用默认站点 ${effectiveSiteId} 的审计日志`
    }
  };
}
...
function buildLineageChain(batchId, db, visited = new Set()) {
  ...
  if (batch.lineage && batch.lineage.transferredFrom) {
    const srcChain = buildLineageChain(batch.lineage.transferredFrom, db, visited);
    for (const id of srcChain) {
      if (!chain.includes(id)) chain.unshift(id);
    }
  }
  if (batch.lineage && batch.lineage.transferredTo && batch.lineage.transferredTo.length) {
    for (const targetId of batch.lineage.transferredTo) {
      const targetChain = buildLineageChain(targetId, db, visited);
      for (const id of targetChain) {
...
function replayBatchToPoint(targetBatchId, targetTime, currentDb, allLogs) {
  ...
  for (const log of relevantLogs) {
    ...
    switch (log.operation) {
      ...
      case "transfer_ship": {
        if (log.details && log.details.sourceBatch) {
          const sb = log.details.sourceBatch;
          if (lineageChain.includes(sb.id)) {
            const b = reconstructedBatches.get(sb.id);
            if (b) {
              if (sb.inTransitQuantityAfter !== undefined) b.inTransitQuantity = sb.inTransitQuantityAfter;
            }
          }
        }
        break;
        ...
        if (log.details && log.details.sourceBatch) {
          ...
          if (lineageChain.includes(sb.id)) {
            ...
            if (b) {
              ...
              if (sb.statusAfter) b.status = sb.statusAfter;
              if (sb.transaction) b.transactions.push(clone(sb.transaction));
              if (!b.lineage) b.lineage = { splitFrom: null, splitTo: [], mergedFrom: [], mergedInto: null, transferredFrom: null, transferredTo: [] };
              if (log.details.targetBatch && log.details.targetBatch.id) {
                if (!b.lineage.transferredTo) b.lineage.transferredTo = [];
                if (!b.lineage.transferredTo.includes(log.details.targetBatch.id)) {
                  b.lineage.transferredTo.push(log.details.targetBatch.id);
                }
              }
            }
          }
      ...
      case "transfer_cancel": {
        if (log.details && log.details.sourceBatch) {
          ...
          if (lineageChain.includes(sb.id)) {
            const b = reconstructedBatches.get(sb.id);
            if (b && sb.inTransitQuantityAfter !== undefined) {
              b.inTransitQuantity = sb.inTransitQuantityAfter;
            }
          }
        }
        break;
...
function summarizeOperation(log, targetBatchId) {
  ...
  switch (log.operation) {
    ...
    case "location_slot_assign": {
      const loc = d.sectionId && d.boxId ? `${d.sectionId}/${d.boxId}:${d.slotIndex || ""}` : `${d.boxId || ""}:${d.slotIndex || ""}`;
      if (d.changeType === "clear") {
        return `清空槽位 ${loc}，原批次 ${d.previousBatchId || "空"} 已移出（批次历史保留）`;
      }
      if (d.changeType === "assign") {
        const syncInfo = [];
        if (d.containerChanged) syncInfo.push(`container: ${d.originalContainer || "空"} → ${d.newContainer}`);
        if (d.sectionChanged) syncInfo.push(`section: ${d.originalSection || "空"} → ${d.newSection}`);
        const syncStr = syncInfo.length ? `，已同步[${syncInfo.join(", ")}]` : "";
        return `分配槽位 ${loc}：${d.previousBatchId || "空"} → ${d.newBatchId || "空"}${syncStr}`;
      }
      return `槽位分配：${loc} ${d.previousBatchId || "空"} → ${d.newBatchId || "空"}`;
    }
    case "transfer_create":
      return d.transfer
        ? `创建调拨单 ${d.transfer.id}：${d.transfer.sourceSiteId} → ${d.transfer.targetSiteId}，数量 ${d.transfer.quantity} 粒`
        : `创建调拨单`;
    case "transfer_ship":
      return d.transfer ? `调拨单 ${d.transfer.id} 已发出，数量 ${d.transfer.quantity} 粒` : `调拨单已发出`;
    case "transfer_receive": {
      const mode = d.mergeMode ? "合并" : "新建";
      return d.transfer
        ? `调拨单 ${d.transfer.id} 已接收（${mode}模式），数量 ${d.transfer.quantity} 粒`
        : `调拨单已接收`;
    }
    case "transfer_cancel":
      return d.transfer ? `调拨单 ${d.transfer.id} 已取消` : `调拨单已取消`;
    default:
