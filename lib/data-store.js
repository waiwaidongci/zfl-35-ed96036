...
async function mutate(options) {
  ...
  let locResult = null;
  if (locMutator && locDb) {
    locResult = await locMutator(locDb, result, db);
  ...
  if (locResult && locResult.additionalAffectedBatchIds) {
    for (const id of locResult.additionalAffectedBatchIds) finalAffectedIds.add(id);
  }
