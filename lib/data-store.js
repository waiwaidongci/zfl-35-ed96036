...
function acquireLock() {
  const release = () => {};
  const newLock = globalLock.then(() => release);
  globalLock = newLock;
  return newLock;
}
...
function incrementVersion(wrapped) {
  const { data, version } = unwrapMetadata(wrapped);
  return wrapWithMetadata(data, wrapped._dataType || "unknown", version + 1);
}
...
async function executeAtomicTransaction(writes) {
  const results = [];
...
async function mutate(options) {
  ...
  try {
    ...
    writes.push(async function writeRareSeeds() {
      const saveResult = await saveDbWithVersion(db, beforeVersions.dataVersion);
      if (saveResult.error) return saveResult;
      beforeVersions.dataVersion = saveResult.newVersion;
      return saveResult;
    });

    if (locMutator && locDb) {
      writes.push(async function writeLocations() {
        const saveResult = await saveLocDbWithVersion(locDb, beforeVersions.locVersion);
        if (saveResult.error) return saveResult;
        beforeVersions.locVersion = saveResult.newVersion;
        return saveResult;
      });
    }

    writes.push(async function writeAuditLogs() {
      const saveResult = await saveAuditWithVersion(audit, beforeVersions.auditVersion);
      if (saveResult.error) return saveResult;
      beforeVersions.auditVersion = saveResult.newVersion;
      return saveResult;
    });

    const txResult = await executeAtomicTransaction(writes);
    if (!txResult.success) {
      return {
        error: "transaction_failed",
        message: txResult.message || "事务执行失败，所有变更已回滚",
        cause: txResult.error,
        failedAt: txResult.failedAt,
    ...
    const finalVersions = {
      dataVersion: beforeVersions.dataVersion,
      locVersion: beforeVersions.locVersion,
      auditVersion: beforeVersions.auditVersion
    };

    let finalResult;
    if (locMutatorResult) {
      finalResult = { ...locMutatorResult, _auditId: entry.id, _versions: finalVersions };
      if (result && result.createdBatchIds) {
      ...
      finalResult = { ...(result || {}), _auditId: entry.id, _versions: finalVersions };
      delete finalResult.additionalAffectedBatchIds;
    }
    return finalResult;
  } finally {
