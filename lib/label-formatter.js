...
function formatLocation(section, container, slotLocations) {
  ...
  if (section && container) {
    return `未分配（历史：${section} / ${container}）`;
  }
  return "未分配";
}
...
function calculateAvailableQuantity(batch) {
  ...
  const inTransit = Number(batch.inTransitQuantity || 0);
  return Math.max(0, quantity - frozen - inTransit);
}
...
export function buildLabel(batch, slotLocations, siteMap) {
  ...
  return {
    ...
    inTransitQuantity: Number(batch.inTransitQuantity || 0),
