...
```bash
# 运行完整测试套件（版本控制 + 锁串行化 + 调拨集成 + HTTP 冒烟）
npm test
```

测试套件基于 Node.js 内置 `node:test` 运行，包含四个阶段：

| 阶段 | 说明 |
|------|------|
| 版本控制与原子事务 | 验证元数据包装、原子写入、版本递增、乐观锁冲突检测 |
| JSON版本读写与锁串行化 | 验证版本冲突审计、并发互斥锁串行化、跨文件事务 |
| 调拨流程集成回归 | 完整创建/发运/收货/取消流程，含库位、流水、谱系验证 |
| HTTP 冒烟测试 | 启动真实服务器，验证核心端点返回结构和状态码 |

### 数据隔离

测试运行前自动备份 `data/` 目录下所有 JSON 文件（`rare-seeds.json`、`locations.json`、`audit-logs.json`），测试完成后无论成功或失败均会恢复原始数据，不会污染真实业务数据。
...
|--------|------|
| `0` | 全部通过 |
| `1` | 存在失败用例，适合 CI 管道 `npm test && deploy` |
...
node test-fixes.js
node test-transfer-integration.js
```
...
| 场景 | 端口 | 说明 |
|------|------|------|
| 正常启动 | `3035` | 默认，可通过 `PORT` 环境变量修改 |
| HTTP 冒烟测试 | `3036` | 测试专用，避免与运行中的服务冲突 |

### 数据恢复

如果测试进程异常中断（如 `SIGKILL`），自动恢复可能未执行。此时可手动恢复：

```bash
# data/ 目录下的 .bak 文件为测试前备份（如存在）
# 最安全的方式：用 git 恢复
git checkout -- data/
```

支持多冷库站点、批次、温度、取样、萌发实验、库存流水、负库存拦截、批次备注、人工复核和取样预约。
